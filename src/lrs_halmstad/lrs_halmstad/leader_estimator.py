...
from lrs_halmstad.follow_math import coerce_bool, quat_from_yaw, wrap_pi, yaw_from_quat
...
class LeaderEstimator(Node):
    ...
    def on_external_detection(self, msg: String) -> None:
        ...
        try:
            ...
            if len(obb_raw) == 4:
                obb_corners = tuple((float(pt[0]), float(pt[1])) for pt in obb_raw)
            obb_heading_yaw = payload.get("obb_heading_yaw", None)
            self.last_external_det = Detection2D(
